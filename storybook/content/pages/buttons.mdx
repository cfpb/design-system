...
>
  <Variation
    ...
    description={`Use a primary button for an action that goes to the next step. Avoid using multiple primary buttons on a single page; there can be multiple secondary buttons per page.`}
    code={`<button class="a-btn" title="Primary">Primary</button>`}
    implementation={`For accessibility reasons, use the semantic \`<button>\` instead of a link
  when possible.
...
<Variation
  ...
  description={`Use a secondary button for actions that happen on the current page.`}
  code={`<button class="a-btn a-btn--secondary" title="Secondary">Secondary</button>`}
/>

<Variation
  title={`Disabled button`}
  code={`<button class="a-btn a-btn--disabled" title="Disabled" disabled>Disabled</button>`}
/>

<Variation
  title={`Destructive button`}
  description={`Although a destructive action can be styled as a button, we recommend using the [destructive link](https://cfpb.github.io/design-system/components/links#destructive-link) style for consistency across cf.gov products.`}
  code={`<button class="a-btn a-btn--warning" title="Destructive">Destructive</button>`}
/>

<Variation
  title={`Button with icon`}
  description={`An icon should appear after the text it represents. The only exception is the back button, where the icon appears before the button’s text. Each icon should be used exclusively and consistently for one action.`}
  code={`<div class="m-btn-group">
      <button class="a-btn a-btn--secondary">{% include icons/left.svg %}
        <span>Go back</span>
      </button>
      <button class="a-btn a-btn--secondary">
...
<Variation
  title={`Button with animated icon`}
  description={`Use an animated icon in a button to reassure the user that the action they are attempting to perform is functioning as intended.
  The icon can be hidden when the action is complete by adding the \`a-btn--hide-icon\` class to the button.
  `}
  code={`<button class="a-btn">
      <span>Submit your complaint</span>
      {% include icons/updating.svg %}
  </button>`}
/>
...
<Variation
  title={`Button group`}
  description={`A button group is an element that combines multiple buttons into a single unit, often used to represent related actions or options. The \`m-btn-group\` class wrapped around a group of buttons will properly space the buttons across screen sizes.`}
  code={`<div class="m-btn-group">
   <button class="a-btn a-btn--secondary">{% include icons/left.svg %}
        <span>Go back</span>
      </button>
...
<Variation
  title={`Full-width button (on x-small screens)`}
  description={`Reduce screen size to see this button in action.`}
  code={`<button class="a-btn a-btn--full-on-xs" title="Default state">Resize to mobile to see effect</button>`}
/>

<Variation
  title={`Button link`}
  description={`A link can be styled as a button, and a button can be styled as a link. Whenever possible, use buttons for actions and links for navigation.`}
  code={`<div class="m-btn-group">
      <a class="a-btn">
          <span>Link styled as a button</span>
          {% include icons/download.svg %}
...
/>
  ...
  <Variation
    title={`Printed button`}
    description={`When a [consumerfinance.gov](https://www.consumerfinance.gov/) page is printed, buttons are presented as links. See [Printed links](https://cfpb.github.io/design-system/components/links#printed-links) on the Links page for specifications.`}
  />
</VariationGroup>
...
<Section title={`Guidelines`}>
  * Use generous white space to bring focus to a button and better make it
  appear actionable. * Write labels in sentence case. * Use verbs and an active
  voice. * Use clear, succinct, and informative language. * Limit the copy
  length to 22 characters.
</Section>

<Section title={`Restrictions`}>
  <GuidancePair
    doText={`<button class="a-btn" title="Test button">Short label</button>`}
    dontText={`<button class="a-btn" title="Test button">This label is much, much too long</button>`}
  />

  <GuidancePair
    doText={`<button class="a-btn">
      {% include icons/left.svg %}
      <span>Back</span>
  </button>`}
    dontText={`<button class="a-btn">
      <span>Back</span>
      {% include icons/left.svg %}
  </button>`}
  />
