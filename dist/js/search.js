...
function displaySearchResults(elm, results, store) {
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    var searchMatchFields = results[i].matchData.metadata[searchMatchWordFragment]; // Remove fields that should never appear as the preview.
...
var results = []; // Get the global search store.

var searchStore = window.searchStore; // Check if the URL has a search term set.

if (searchTerm) {
  document.getElementById('search-box').setAttribute('value', searchTerm); // Get the lunr index.

  var idx = initializeSearchIndex(searchStore); // Perform a search on lunr index.
