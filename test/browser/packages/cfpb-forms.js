...
describe( 'Multiselect', () => {
  ...
  it( 'should show the first multiselect option when opened', async () => {
    ...
    const firstMultiSelectOption = await $( '.a-live_code .o-multiselect_options li:first-child' );
  ...
  it( 'should not show the last multiselect option until the user scrolls to it', async () => {
    ...
    const lastMultiSelectOption = await $( '.a-live_code .o-multiselect_options li:last-child' );
    await lastMultiSelectOption.scrollIntoView();
    const multiselectFieldsetScrollTop = await multiselectFieldset.getProperty( 'scrollTop' );
    // Ensure multiselect has fully expanded
    await browser.pause( 300 );
  ...
  it( 'should correctly filter the multiselect options', async () => {
    ...
    const firstMultiSelectOption = await $( '.a-live_code .o-multiselect_options li[data-option=option1]' );
    const fourthMultiSelectOption = await $( '.a-live_code .o-multiselect_options li[data-option=option4]' );
    const longMultiSelectOption = await $( '.a-live_code .o-multiselect_options li[data-option=option8]' );
  ...
  it( 'should let the user remove a choice', async () => {
    ...
    const multiSelectChoice = await $( '.o-multiselect_choices label[for=test_select__multiple-option1]' );
  ...
  it( 'should let the user add a choice', async () => {
    ...
    const secondMultiSelectOption = await $( '.a-live_code .o-multiselect_options li[data-option=option2] label' );
    await secondMultiSelectOption.click();

    const secondMultiSelectChoice = await $( '.a-live_code .o-multiselect_choices label[for=test_select__multiple-option2]' );
