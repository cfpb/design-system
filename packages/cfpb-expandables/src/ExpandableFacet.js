 ...
 *
 * @class
 * @classdesc Initializes a new ExpandableFacet molecule.
 * @param {HTMLElement} element - The DOM element within which to search
 *   for the molecule.
 * @returns {ExpandableFacet} An instance.
 */
function ExpandableFacet(element) {
  // Internal vars.
  ...
  function init() {
    ...
    _inputTree = new InputTree(_dom.querySelector(`.${InputTree.BASE_CLASS}`));
    _inputTree.init();
    _inputTree.addEventListener('onopen', () => {
      _flyout.getTransition().refresh();
    });
    _inputTree.addEventListener('onclose', () => {
      _flyout.getTransition().refresh();
    });
...
ExpandableFacet.BASE_CLASS = BASE_CLASS;
ExpandableFacet.init = (scope) =>
  instantiateAll(`.${ExpandableFacet.BASE_CLASS}`, ExpandableFacet, scope);

export { ExpandableFacet };
